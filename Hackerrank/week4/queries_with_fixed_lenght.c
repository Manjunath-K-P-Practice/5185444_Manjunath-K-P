...
int* solve(int n,int* arr,int q,int* queries,int* rc){
    int* prev=malloc(n*sizeof(int));
    int* next=malloc(n*sizeof(int));
    int* stack=malloc(n*sizeof(int));
    int* tmp=malloc((n+2)*sizeof(int));
    ...
    for(int i=0;i<n;i++){
        ...
        if(arr[i]<tmp[len]) tmp[len]=arr[i];
    }
    ...
    for(int i=0;i<q;i++) res[i]=tmp[queries[i]];
